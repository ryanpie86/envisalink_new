...
- **Hardware status**: zone *type* and zone *name* (`include_names`/"Include Names") are both confirmed working against real Vista-20P and Vista-21iP hardware. See [docs/zone_discovery.md](docs/zone_discovery.md) for exactly what this does, its safety model, and current limitations (wireless zones aren't supported yet, and an unprogrammed zone's captured name text isn't fully confirmed).
