...
The number of zones/partitions genuinely can't be discovered automatically, but for Honeywell/Vista panels, each zone's *type* and (if one was ever set) its *name* are already stored in the panel's own installer programming, and can be read back the same way an installer reads them off a physical alpha keypad. Set an installer code and select your panel model on the integration's Basic options page -- "Vista 20P (Non-ADT Panels Only)" and "Vista 21iP (Non-ADT Panels Only)" are supported today. This defaults to "I don't know / not listed," which deliberately disables zone discovery: sending this feature's keystroke sequence to an unconfirmed panel model could type nonsense into whatever installer menu that panel actually has, so you must explicitly pick a supported model before the Zone Scan device appears. Once you do, it creates a separate **Zone Scan** device (its own card, linked to your alarm panel) with four controls: three toggle switches -- **Apply Changes**, **Include Names**, **Remove Unused Zones** (all off by default) -- and a **Discover Zone Info** button that runs a scan using whatever those switches are currently set to. With Apply Changes off, a press is always a safe, no-op preview regardless of the other two switches. You can also call the `envisalink_new.discover_zone_info` service directly (Developer Tools > Actions) with `apply`/`include_names`/`remove_unused` set explicitly -- handy for automations/scripts. Either way, it always scans every valid zone number for a Vista-20P/15P (1-64) rather than only the zones you've already configured -- the point of discovery is finding zones you haven't set up yet. A zone that comes back type "00" (Not Used) is otherwise left alone even when applying; turn on "Remove Unused Zones" (or pass `remove_unused: true`, which requires `apply: true`) to also drop such zones from your configured `zone_set`. Zone *type* and zone *name* (`include_names`/"Include Names") are both confirmed working against real Vista-20P hardware -- see [docs/zone_discovery.md](docs/zone_discovery.md) for exactly what this does, its safety model, and its current limitations (wireless zones aren't supported yet, and an unprogrammed zone's captured name text isn't fully confirmed). This briefly puts your panel into installer programming mode, so it requires the partition to be disarmed, and it's new -- please read that doc, and start with everything off except "Include Names" (to preview names safely) before turning on "Apply Changes."
