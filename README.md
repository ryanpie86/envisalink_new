...
See [docs/wireless_zone_diagnostics.md](docs/wireless_zone_diagnostics.md) for an investigation (closed, no feature work planned) into per-zone wireless loop/trouble diagnostics for Honeywell zones, and why full loop-level detail like HA's `alarmdecoder` integration has isn't achievable over the EVL's TPI protocol.
