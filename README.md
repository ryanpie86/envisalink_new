...
The number of zones/partitions genuinely can't be discovered automatically, but for Honeywell/Vista panels, each zone's *type* and (if one was ever set) its *name* are already stored in the panel's own installer programming, and can be read back the same way an installer reads them off a physical alpha keypad. Set an installer code and confirm the panel model (currently only "Vista 20P (Non-ADT Panels Only)" is supported) on the integration's Basic options page, then either click the **Discover Zone Info** button, found alongside the Panic A/B/C buttons under the alarm panel device's Controls, for a quick dry-run preview, or call the `envisalink_new.discover_zone_info` service directly (Developer Tools > Actions) with `apply: false` first to review the results, then `apply: true` to write them in. It always scans every valid zone number for a Vista-20P/15P (1-64) rather than only the zones you've already configured -- the point of discovery is finding zones you haven't set up yet. A zone that comes back type "00" (Not Used) is otherwise left alone even with `apply: true`; pass `remove_unused: true` (requires `apply: true`) to also drop such zones from your configured `zone_set` -- the button can't do this since it only ever runs a dry run. See [docs/zone_discovery.md](docs/zone_discovery.md) for exactly what this does, its safety model, and its current limitations (wireless zones aren't supported yet, and *name* reading is temporarily disabled while the zone-*type* walk is validated against real hardware -- `zone_type` is currently the only field populated). This briefly puts your panel into installer programming mode, so it requires the partition to be disarmed, and it's new -- please read that doc, and test with `apply: false` first, before relying on it.
